...
set logic [file exist Handedness_percentage-efield_$field_value.xvg]

if {$logic == 1} {
  set percentage_outfile [open Handedness_percentage-efield_$field_value.xvg a+]
  set Done [exec grep -w "@" -c -v Handedness_percentage-efield_$field_value.xvg]
 # set percentage_outfile [open Handedness_percentage-efield_0.2.xvg a+]
  puts "$Done"
  puts "ALREADY $Done Frames are calculated"
  mol new   $fname first $Done autobonds yes waitfor all type lammpstrj
#  mol addfile $pname type psf autobonds yes waitfor all
} else {
   mol new   $fname  autobonds yes last 0 waitfor all type lammpstrj
 #  mol addfile $pname type psf autobonds yes waitfor all
   set percentage_outfile [open Handedness_percentage-efield_$field_value.xvg w]
   puts $percentage_outfile "@ title \"Percentage of Handedness\"" 
   flush $percentage_outfile
   puts $percentage_outfile "@ subtitle \"Efield $field_value (V/\\cE\\C)\"" 
   flush $percentage_outfile
   ...
   flush $percentage_outfile
   puts $percentage_outfile "@ view 0.15, 0.15, 1.00, 0.85" 
   flush $percentage_outfile
   ...
   flush $percentage_outfile
   puts $percentage_outfile "@ legend 1.00,0.8" 
   flush $percentage_outfile
...
set N_tail [llength [[atomselect top {type 7 and residue 0}] get index]]
...
set D_index { {5 0 9 10} {5 4 13 14} {1 2 17 18} }
# Dihedral Index to work with           
...
for {set iframe 0} {$iframe < $nframe } {incr iframe} {
   ...
   for {set istack 0} {$istack < 9} {incr istack} {
    ...
    #flush [lindex $outfile $istack] 
     if {${istack} == 0} {
         puts -nonewline $percentage_outfile "[ fortranformat %15.7f [expr {($iframe+$Done)*$dt}]] \t [fortranformat %15.7f [expr ${stack_left_D}*100.0/[expr $Nmols_per_stack*$N_tail]]]"
         flush $percentage_outfile
